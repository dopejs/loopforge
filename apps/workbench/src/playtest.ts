...
export type PlaytestState = {
  ...
  allowed: boolean;
  protocol: { protocol_id: string; created_at: string; build_identity: string } | null;
  report: {
    evidence_id: string;
    revoked: boolean;
    revoked_at: string;
  ...
  /** Generated by the core; the user must not type or guess it. */
...
export function emptyReport(buildIdentity = ""): PlaytestReport {
  return {
...
export function serializeReport(form: PlaytestReport): Record<string, unknown> {
  ...
  for (const field of PLAYTEST_TEXT_FIELDS) payload[field] = form[field];
...
export function revokeReport(
  projectRoot: string,
  evidenceId: string,
  reason: string
): Promise<PlaytestState> {
  return invoke<PlaytestState>("agent_playtest_revoke", {
    projectPath: projectRoot,
    evidenceId,
    reason
  });
